...
void main()
{
    ...
    const float size = easeOutElastic(animationProgress) * (1 - sizeStrength) + sizeStrength;

    const vec2 dMouse = uPosition - lastClickPosition;
    vAnimationOffset = sign(dMouse) * min(vec2(uSize - uSize * (sizeStrength)), abs(dMouse)) * (1 - easeOutElastic(animationProgress));
